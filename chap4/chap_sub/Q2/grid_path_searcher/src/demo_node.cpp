...
void trajectoryLibrary(const Eigen::Vector3d start_pt, const Eigen::Vector3d start_velocity,
  ...
  {  // acc_input_ax
    ...
    {  // acc_input_ay
      ...
      {  // acc_input_az
        ...
        cout << "current i j k is " << i << ' ' << j << ' ' << k << endl;
        // cout << "acc_input " << '=' << acc_input(0) << endl;

        for (int step = 0; step <= _time_step; step++)
        {
          /*
          STEP 1: finish the forward integration, the modelling has been given in the document
          the parameter of forward integration: _max_input_acc|_discretize_step|_time_interval|_time_step   all have
          been given
        ...
        */
        ...
        cout << "Trajectory cost is " << Trajctory_Cost << endl << endl;
        // cout << "target_pt is " << target_pt(0) << endl;

        // input the trajetory in the trajectory library
        TraLibrary[i][j][k] = new TrajectoryState(Position, Velocity, Trajctory_Cost);

        // if there is not any obstacle in the trajectory we need to set 'collision_check = true', so this trajectory is
        // useable
        if (collision)
          TraLibrary[i][j][k]->setCollisionfree();

        // record the min_cost in the trajectory Library, and this is the part pf selecting the best trajectory cloest
        // to the planning traget
