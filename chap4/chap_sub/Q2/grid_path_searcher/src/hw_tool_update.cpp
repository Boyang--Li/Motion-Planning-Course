                                ...
                                Eigen::Vector3d _target_position)
{
  ...
  */
  ...
  optimal_cost = T + (pow(alpha1, 2) * pow(T, 3)/3 + alpha1 * beta1 * pow(T, 2) + pow(beta1, 2) * T) +
                  (pow(alpha2, 2) * pow(T, 3)/3 + alpha2 * beta2 * pow(T, 2) + pow(beta2, 2) * T) +
                  (pow(alpha3, 2) * pow(T, 3)/3 + alpha3 * beta3 * pow(T, 2) + pow(beta3, 2) * T);
  cout << "Trajectory cost is " << optimal_cost << endl;
